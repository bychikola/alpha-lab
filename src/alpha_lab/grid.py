...
полного пути (см. SECONDS_PER_CONFIG), а не «узнаем через час».
"""
...
# Измеренная стоимость полного пути одной конфигурации: причинностный
# harness + generate + run_backtest + validate. Замер 2026-09-11, реальные
# данные BTCUSDT 1h 2022–2025, 35 064 бара, три конфигурации: медиана 2.32 с
# (2.31–2.40). Внутри: harness 1.58–1.76 с, generate + backtest ≈0.08 с,
# validate 0.66–0.67 с (из них permutation-тест 646 мс).
#
# Прежняя оценка 0.710 с не включала harness и была оптимистична в 3–4 раза —
# молчаливо заниженная оценка хуже отсутствия оценки. Здесь замер полного
# пути, поэтому размер предупреждения соответствует прогону.
#
# Стоимость материально зависит от длины ряда (тот же период, другие
# таймфреймы): 4h / 8 766 баров — 1.34 с, 1d / 1 461 бар — 1.54 с (harness
# даёт пол ~1.4 с), 15m / 140 256 баров — 8.68 с. Линейным коэффициентом это
# не масштабируется (на коротких рядах доминирует фиксированная стоимость
# harness), поэтому оценка привязана к 35k барам, а текст предупреждения
# называет базу замера и направление сдвига на другой длине ряда.
SECONDS_PER_CONFIG = 2.32
# Загрузка + ресемпл баров: 22.2 с на (символ, таймфрейм), один раз; замеры
# 2026-09-11 дали 21.4–22.7 с на 4 года истории.
SECONDS_PER_LOAD = 22.2
# Порог громкого предупреждения: 1000 конфигураций — это ≈ 39 минут только
# счёта (2.32 с × 1000, плюс загрузки). Ниже порога свип укладывается в
# короткую сессию.
...
def estimate_wall_clock(n_configs: int, n_loads: int) -> float:
    ...
    Счёт — по полному пути конфигурации (harness причинности, generate,
    backtest, validate), измерен на 1h-ряде ~35 000 баров. На ряде другой
    длины оценка сдвигается: см. SECONDS_PER_CONFIG.
    """
...
def size_warning(n_configs: int, n_loads: int) -> str | None:
    ...
    на пару. Оценка — по измеренной стоимости полного пути
    (SECONDS_PER_CONFIG/SECONDS_PER_LOAD), а не по интуиции; текст называет,
    что входит в оценку, на какой длине ряда она измерена и что это оценка.
    """
    ...
    return (
        ...
        f"≈ {_format_duration(total)} машинного времени. Оценка: {count} × "
        f"{SECONDS_PER_CONFIG:.2f} с полного пути одной конфигурации "
        f"(причинностный harness + generate/backtest + validate; замер на "
        f"ряде ~35 000 баров) + {n_loads} загрузк(и) баров × "
        f"{SECONDS_PER_LOAD:.1f} с. На ряде короче 35 000 баров счёт дешевле "
        f"(4h ≈ 1.3 с), на более длинном — дороже (15m ≈ 8.7 с). Это "
