...
def build_report(verdict: Verdict, equity: pd.Series, close: pd.Series,
    ...
    verdict_payload = {
        ...
        # Неприменимые гейты (spec 6.6) — отдельный от reasons и warnings канал:
        # не провал и не проход, а невозможность проверки для этой книги. Пустой
        # список (не null), как и у warnings: «неприменимых нет» отличимо от
        # «поле не передано». alive при непустом списке не выносится.
        "inapplicable": list(getattr(verdict, "inapplicable", ()) or ()),
