...
Параметры подобраны замером, а не подгонкой порогов:
  * theta=0.02 (полураспад ≈ 35 баров) — возврат укладывается в окно z-скора
    (20 баров) и в max_bars=500; на 10 seed'ах подряд вердикт alive=True,
    DSR 0.95..1.00, p 0.001..0.028, сделок 269..321;
  * theta=0.002 (полураспад ≈ 347 баров) — возврат много медленнее окна
    стратегии: на 10 seed'ах подряд alive=False при 349..400 сделках, то есть
